...
This page documents the engine's module structure (~60,000 lines of R across 47 files). **No source
...
|---|---|---|---|
| `05_mg_*` | MG signal | Latent agent ecology, order parameters, frequency collapse (selection / consensus) | A / B / C |
...
| `12_execution_intent_*` | Intent | Global action priority, quantization / min-delta policy, pending latch | A / B / C |
| `13_execution_realized_*` | Execution | t+1 fill, cost model — the only writer of realized position | A / B / C |
...
| `14_postrun_export` | 52 canonical CSV surfaces, written from committed state |
| `15_diagnostic` | 111 derived attribution surfaces for full signal→fill tracing |
